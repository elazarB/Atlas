...
export const createInfo1 = (_ar) => {
  document.querySelector("#id_flag").src = _ar[0].flags.png;
  document.querySelector("#id_name").innerHTML = _ar[0].name.common;
  document.querySelector("#id_pop").innerHTML = _ar[0].population.toLocaleString();
  document.querySelector("#id_region").innerHTML = _ar[0].continents[0];
  ...
  if (_ar[0].borders) {
    ...
    _ar[0].borders.forEach(item => {
      if (!(objNamesByCodes(item) == "Palestine")) {
        ...
        borders.className = " btn badge text-bg-info bg-opacity-50 col-2 me-1 mb-1"
